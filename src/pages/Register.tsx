...
import { Link, useNavigate } from "react-router-dom"; // Removed BrowserRouter import
// The original import for useAuthStore could not be resolved, so we are providing a mock implementation
// to ensure the component runs correctly in this environment.

// --- MOCK AuthStore Implementation ---
const useAuthStore = () => ({
  setUser: (data) => {
    console.log("Mock AuthStore: Setting user data:", data);
  },
  isLogin: false,
});
// ------------------------------------
...
const RegistrationForm = () => {
  ...
  const updateHostelList = (form) => {
    // Note: We use the `form` object passed from the onChange event handler to get current radio/select values
...
// Removed AppWrapper to prevent nested Router error.
